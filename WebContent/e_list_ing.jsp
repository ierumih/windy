...
<%
	List<Event> Elisting = (List<Event>) request.getAttribute("Elisting");
	...
	<div id="wrap">
		<div class="board_head">
			<div class="title">
				<h2>이벤트</h2>
				<ul class="event">
					<li><a href="eventlisting.b" class="now">진행중인 이벤트</a></li>
					<li><a href="eventlistend.b" class="not">종료된 이벤트</a></li>
				</ul>
		...
		<div class="pp">
			<ul class='row' style="height:845px;">
				...
				<%
					for (int i = 0; i < Elisting.size(); i++) {

						out.println("<a href='eventdetailing.b?e_num=" + Elisting.get(i).getE_num() + "'><li><img src="
								+ Elisting.get(i).getE_sum() + ">");
						out.println("<div>" + Elisting.get(i).getE_title() + "</div>");
						out.println("<p> 이벤트기간 <span>" + Elisting.get(i).getS_date() + " ~ " + Elisting.get(i).getE_date()
								+ "</span></p></a></li>");
					}
				%>
			</ul>
			...
			<table>
				<tr>
					...
					<td><input type='button' value='이전' class='
						search' onclick="location.href='eventlisting.b?e_date=<%=eventpage.getTime()%>&p=<%=p - 1%>'"></td>
					<%
						}
					%>
					<td><ul class="num">
							<%
								for (int i = (p - 5); i < (p + 5); i++) {
									if ((i > 0) && (i <= pag)) {
										if (i != p) {
							%>
							<a href='eventlisting.b?e_date=<%=eventpage.getTime()%>&p=<%=i%>'><li><%=i%></li></a>
							<%
								} else {
							%>
							<li id='cp'><%=i%></li>
					...
					<td><input type='button' value='다음' class='search'
						onclick="location.href='eventlisting.b?e_date=<%=eventpage.getTime()%>&p=<%=p + 1%>'"></td>
					<%
						}
					%>
				</tr>
